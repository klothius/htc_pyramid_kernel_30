...
/* Buffer with meta*/
#define PCM_BUF_SIZE		(8192 + sizeof(struct meta_in))

/* Maximum 10 frames in buffer with meta */
#define FRAME_SIZE		(1 + ((35+sizeof(struct meta_out_dsp)) * 11))
				...
				unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_START: {
		...
		if (rc < 0) {
			pr_aud_err("%s:session id %d: cmd qcelp media format block\
				failed\n", __func__, audio->ac->session);
			break;
		}
	...
	case AUDIO_SET_QCELP_ENC_CONFIG: {
		...
		if (cfg.min_bit_rate > 4 ||
			 cfg.min_bit_rate < 1) {
			pr_aud_err("%s:session id %d: invalid min bitrate\n",
					__func__, audio->ac->session);
			rc = -EINVAL;
			break;
		}
		if (cfg.max_bit_rate > 4 ||
			 cfg.max_bit_rate < 1) {
			pr_aud_err("%s:session id %d: invalid max bitrate\n",
					__func__, audio->ac->session);
			rc = -EINVAL;
			break;
		}
