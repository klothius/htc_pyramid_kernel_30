...
static struct msm_bus_paths bw_level_tbl[] = {
	[0] =  BW_MBPS(2200), /* At least 103 MHz on bus. */
	[1] = BW_MBPS(2600), /* At least 167 MHz on bus. */
	[2] = BW_MBPS(3000), /* At least 251 MHz on bus. */
	[3] = BW_MBPS(3400), /* At least 340 MHz on bus. */
	[4] = BW_MBPS(3800), /* At least 450 MHz on bus. */
};
...
static struct clkctl_acpu_speed acpu_freq_tbl_1836mhz[] = {
  ...
  { {1, 1}, 1566000, ACPU_SCPLL, 0, 0, 1, 0x1D, L2(23), 1225000, 0x03006000},
  { {1, 1}, 1620000, ACPU_SCPLL, 0, 0, 1, 0x1E, L2(23), 1250000, 0x03006000},
  { {1, 1}, 1674000, ACPU_SCPLL, 0, 0, 1, 0x1F, L2(23), 1275000, 0x03006000},
  { {1, 1}, 1728000, ACPU_SCPLL, 0, 0, 1, 0x20, L2(23), 1300000, 0x03006000},
  { {1, 1}, 1782000, ACPU_SCPLL, 0, 0, 1, 0x21, L2(23), 1325000, 0x03006000},
...
static unsigned int __init select_freq_plan(void)
{
	...
	if (speed_bin == 0x1) {
		...
		case 0x0:
		case 0x7:
			pr_err("ACPU PVS: Slow\n");
			break;
		case 0x1:
			pr_err("ACPU PVS: Nominal\n");
			break;
		case 0x3:
			pr_err("ACPU PVS: Fast\n");
			break;
		default:
    ...
    max_khz = 1944000;
    acpu_freq_tbl = acpu_freq_tbl_1836mhz;
